...
namespace Fs.Fox.CAD.Diagnostics.Test
{
    ...
    ///
    /// </summary>
    internal class CategoryTests : MgdDbgTestFuncs
    {
        ...
        DwgStats ()
        {
            ...
            if (dbox.ShowDialog() == DialogResult.OK) {
                ...
                PublishReportResources(dbox.FileName);
            }
        }
        ...
        /// <summary>
        /// Extracts only the report-browser files owned by this diagnostic
        /// assembly after the XML report has been written successfully.
        /// A resource failure does not hide the usable XML output.
        /// </summary>
        private static void
        PublishReportResources(string reportPath)
        {
            Utils.AcadUi.PrintToCmdLine(string.Format("\nDWG statistics XML: {0}", reportPath));

            try {
                string browserPath = ReportResourceExtractor.ExtractBeside(reportPath);
                Utils.AcadUi.PrintToCmdLine(string.Format("\nReport browser: {0}", browserPath));
            }
            catch (System.Exception exception) {
                Utils.AcadUi.PrintToCmdLine(string.Format(
                    "\nReport browser is not available: {0}", exception.Message));
            }
        }
