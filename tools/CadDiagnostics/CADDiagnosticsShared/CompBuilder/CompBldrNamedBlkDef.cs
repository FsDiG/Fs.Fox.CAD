...
namespace Fs.Fox.CAD.Diagnostics
{
	...
	internal class CompBldrNamedBlkDef : CompBldr
	{
	    protected string    m_blkName;

		public
		CompBldrNamedBlkDef(Database db, string blkName)
		:   base(db)
		{
		    m_blkName = blkName;
		}
        ...
        SetCurrentBlkTblRec()
        {
            ...
            if (m_blkDefId.IsNull) {
                m_blkRec = DefineNewBlockRec(m_blkName);
            }
            else {
