...
public sealed class DBTrans : IDisposable
{
    ...
    public BlockTableRecord CurrentSpace
    {
        get
        {
            if (_currentSpace is null || Database.CurrentSpaceId != _lastCurrentSpaceId)
            {
                _lastCurrentSpaceId = Database.CurrentSpaceId;
                _currentSpace = BlockTable.GetRecord(Database.CurrentSpaceId)!;
            }
        ...
        if (disposing)
        {
            ...
            if (Transaction != null)
            {
                ...
                else
                {
                    ...
                    // 否则取消所有的修改
                    Transaction.Abort();
                    if (Editor is not null && vtr is not null)
