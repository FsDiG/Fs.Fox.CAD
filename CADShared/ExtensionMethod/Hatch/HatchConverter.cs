...
public class HatchConverter
{
    ...
    static void HatchLoopIsCurve2d(HatchLoop loop, HatchConverterData hcData)
    {
        ...
        foreach (Curve2d curve in loop.Curves)
        {
            ...
            if (curveIsClosed == loop.Curves.Count)
                hcData.PolyLineData.Add(new BulgeVertexWidth(curve.EndPoint));
        }
    }
    ...
    public ObjectId CreateBoundarysAndHatchToMsPs(BlockTableRecord btrOfAddEntitySpace,
        bool boundaryAssociative = true, bool createHatchFlag = true)
    {
        var boEnts = CreateBoundary();
        boEnts.ForEach(ent => { BoundaryIds.Add(btrOfAddEntitySpace.AddEntity(ent)); });
    ...
    void ResetBoundary(Hatch hatch, bool boundaryAssociative = true)
    {
        ...
        for (var i = 0; i < BoundaryIds.Count; i++)
        {
            obIds.Clear();
            obIds.Add(BoundaryIds[i]);
            // 要先添加最外面的边界
            hatch.AppendLoop(i == 0 ? HatchLoopTypes.Outermost : HatchLoopTypes.Default, obIds);
        }
