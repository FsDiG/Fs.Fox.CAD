...
| 移动或重组 `CADShared` | [单程序集逻辑模块化计划](logical-modularization-plan.md)、[Issue #25](https://github.com/FsDiG/Fs.Fox.CAD/issues/25) | 首轮模块化已由 PR #100 合入；计划中的数量和路径表是 Phase A 快照，实时映射以 `main` 的项目清单和模块基线为准。 |
...
| 状态 | 稳定 ID | 文档 | 使用边界 |
| --- | --- | --- | --- |
| `current` | `plan.cad-modules` | [单程序集逻辑模块化执行计划](logical-modularization-plan.md) | 首轮实施已完成；作为结构约束和审计记录继续保留，数量和路径表是 Phase A 快照，不进入公共站点。 |
...
| 状态 | 稳定 ID | 文档 | 当前入口 |
| --- | --- | --- | --- |
| `superseded` | `proposal.cad-modules` | [渐进式模块化重构建议](refactoring-proposal.md) | [单程序集逻辑模块化执行计划](logical-modularization-plan.md)和 Issue #25。 |
