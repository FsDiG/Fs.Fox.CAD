...
`CADShared.projitems` 显式列出全部正式共享编译项，并通过 `FsFoxModule` 和 `FsFoxOrder` 记录九个逻辑模块及稳定编译顺序；当前数量、源码哈希和边界债务以 `Build/CADSharedModuleBaseline.json` 及模块守卫结果为准。目录只表达源码所有权，不改变公共命名空间，也不意味着九个独立 DLL；设计边界和实施记录见[单程序集逻辑模块化执行计划](logical-modularization-plan.md)。
