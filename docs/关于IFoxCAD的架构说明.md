...
1. 新文件先按主要扩展目标或主要状态归属，再检查它拥有的资源生命周期、产生的外部副作用以及可以落入的最低依赖层；目录接近或类型后缀相同不能单独决定归属。
2. 不再以提高“一类型一文件”覆盖率为目标。只有明确改善所有权、兼容性或维护成本时才拆分既有文件，并保持公共类型身份和编译顺序等兼容面。
3. 模块边界调整从最新 `main` 建立短期分支，通过独立 Issue/PR 说明收益、依赖变化和验收证据；不得继续使用历史分支 `refactor/cad-modules`。
4. 只有出现真实独立消费者、形成稳定且可检查的依赖方向，并准备好相应 AutoCAD/ZWCAD 宿主验收矩阵后，才重新评估拆分 DLL 或 NuGet 包。
