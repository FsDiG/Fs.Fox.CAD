...
## 4. Fs.Fox.CAD 的 2025/2026 发布目标决策

`Fs.Fox.CAD` 以现有 ZWCAD 2025 构建产物作为 ZWCAD 2025 和 ZWCAD 2026 的共享发布目标：
...
- 继续使用 `TestZcad2025` 作为 2025/2026 共享产物的构建验证入口。

该决策建立在厂商声明的 2025/2026 接口兼容性之上。当前约定的验收环境不提供 ZWCAD 2025/2026 真实宿主，本项目也暂不要求该代际的宿主测试，因此 `ZW_2025` 明确为 Build only。这个范围决定不等于宿主通过，也不把厂商的接口兼容声明提升为本项目的运行证据。
...
自动构建只能验证源代码和引用关系，不能证明 .NET Framework 插件能在 ZWCAD 2025/2026 中正确绑定、加载和运行。当前验收矩阵不包含这两个真实宿主，`TestZcad2025` 只用于构建验证。
...
| 编译目标 | 测试程序集 | 实际宿主 | 当前状态 |
| --- | --- | --- | --- |
| ZRX 2025 | `TestZcad2025.dll` | 无 | Build only；不计划真实宿主测试 |

如果将来重新把 ZWCAD 2025 或 2026 纳入验收范围，应新建或更新对应 Issue，并至少记录 `VERNUM`、Windows/.NET Framework 版本、测试 DLL SHA-256、Git commit、`NETLOAD`/命令结果、依赖解析、核心数据库操作和实际使用的 UI/自动加载入口。在此之前，文档、Issue 和发布记录都应保留“未做真实宿主验证”的边界，不维护一个看似待执行的宿主清单。

## 10. 资料边界

- 本文是版本兼容性和项目决策摘要，不替代 ZRXSDK 头文件、托管程序集元数据、官方示例或对应年度发布说明。
- CHM 对“2025 编译产物可直接用于 2026”的说明尚未通过本项目真实插件验证，特别是第三方依赖、界面组件和自动加载行为；当前明确接受这一未验证边界。
